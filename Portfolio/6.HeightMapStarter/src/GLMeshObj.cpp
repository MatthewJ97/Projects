...
FBXModel::FBXModel(const char * fileName, glm::mat4 model, bool ani) {
	m_FbxModel = new FBXFile();
	m_FbxModel->load(fileName, FBXFile::UNITS_CENTIMETER);
	m_modelM = model;
	m_animationTime = animationStart;
...
void FBXModel::draw(Camera* camera, std::vector<Light*> lights) {
	
	glUseProgram(m_shader);


	unsigned int mpLoc = glGetUniformLocation(m_shader, "projection");
	glUniformMatrix4fv(mpLoc, 1, GL_FALSE, &camera->m_projectionMatrix[0][0]);
	unsigned int mvLoc = glGetUniformLocation(m_shader, "view");
	glUniformMatrix4fv(mvLoc, 1, GL_FALSE, &camera->m_viewMatrix[0][0]);
	unsigned int mmLoc = glGetUniformLocation(m_shader, "model");
	glUniformMatrix4fv(mmLoc, 1, GL_FALSE, &m_modelM[0][0]);

	glUniform1i(glGetUniformLocation(m_shader, "numLights"), lights.size());
	glUniform3fv(glGetUniformLocation(m_shader, "lightPosition"), lights.size(), &lights[0]->m_Pos.x);
	glUniform3fv(glGetUniformLocation(m_shader, "lightColor"), lights.size(), &lights[0]->m_Col.r);
	glUniform1fv(glGetUniformLocation(m_shader, "lightAmbientStrength"), lights.size(), &lights[0]->m_AmbientStrength);

	glUniform3fv(glGetUniformLocation(m_shader, "CameraPos"), 1, &camera->m_Pos[0]);
	glUniform3fv(glGetUniformLocation(m_shader, "SpecColor"), lights.size(), &lights[0]->m_specColor.r);
	glUniform1fv(glGetUniformLocation(m_shader, "SpecPow"), lights.size(), &lights[0]->m_specPow);
...
{
	if (animationStart > m_animationTime)m_animationTime = animationStart;

	m_animationTime += deltaTime;

	if (m_animationTime >= animationEnd) m_animationTime = animationStart;

	if (m_FbxModel->getSkeletonCount() <= 0)return;

	FBXSkeleton* skeleton = m_FbxModel->getSkeletonByIndex(0);
	FBXAnimation* animation = m_FbxModel->getAnimationByIndex(0);
	float animationPoint = deltaTime;
	skeleton->evaluate(animation, m_animationTime);
...
void FBXModel::update(float deltaTime, FBXFile* ani)
{
	/*if (animationStart > m_animationTime)m_animationTime = animationStart;*/
	...
	FBXSkeleton* skeleton = m_FbxModel->getSkeletonByIndex(0);
	FBXAnimation* animation = ani->getAnimationByIndex(0);
	skeleton->evaluate(animation, m_animationTime);
	for (unsigned int bone_index = 0; bone_index < skeleton->m_boneCount; bone_index++)
	{
		skeleton->m_nodes[bone_index]->updateGlobalTransform();
...
void FBXModel::SetAniStart(float start)
{
	animationStart = start;
}

void FBXModel::SetAniEnd(float end)
{
	animationEnd = end;
}

void FBXModel::SetAniStartEnd(float start, float end)
{
	animationStart = start;
	animationEnd = end;
}
...
BoundingSphere::BoundingSphere(glm::vec3 centerp, float radi) : center(centerp), radius(radi)
{
}
...
void BoundingSphere::fit(const std::vector<glm::vec3>& points)
{
	...
	center = (min + max) * 0.5f; radius = glm::distance(min, center);
}
